...
第一步：查询每个部门最高薪水(按照部门编号分组，找出每一组最大值)
...
第二步：找出每个部门的平均薪水的等级，以上t表和salgrade表连接  
...
### 24. 列出薪资等于部门 30 中员工的薪金的其他员工的姓名和薪金.
...
### 25. 列出薪资高于在部门 30 工作的所有员工的薪金的员工姓名、薪金、部门名称
...
请按照年龄（age）分组，查出不同组的平均分和人数，分数（score）低于 60 分的人不参与分组，如果年龄分组的人数小于 3，那么这个年龄段的人也不参与分组，按照上面要求写出 sql。
...
### 2. 有两张表结构一样，数据都在 400 万左右，现在想用 b 表的数据更新 a 表，并把 b 表中 a 表没的记录加入到 a 表，请写出最优的 sql。

```mysql
UPDATE a
JOIN b ON a.id = b.id  -- 假设 id 是唯一标识
SET a.column1 = b.column1,  -- 更新需要更新的列
    a.column2 = b.column2,  -- 假设 column1 和 column2 是你需要更新的列
    a.column3 = b.column3
WHERE a.column1 <> b.column1  -- 只更新有差异的记录，避免不必要的写操作
   OR a.column2 <> b.column2
   OR a.column3 <> b.column3;
   
   
INSERT INTO a (id, column1, column2, column3)  -- 插入的列
SELECT b.id, b.column1, b.column2, b.column3
FROM b
LEFT JOIN a ON b.id = a.id
WHERE a.id IS NULL;  -- 筛选出 a 表中不存在的记录
```
