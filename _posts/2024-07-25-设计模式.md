...
只创建一个实例，避免一个类频繁的创建、销毁多个实例，节省系统资源。
...
public class Singleton {
  ...
  public static Singleton getUniqueInstance() {
    if (uniqueInstance == null) { // 锁前检查是否已经有实例
      synchronized (Singleton.class) {
        if (uniqueInstance == null) { // 锁后检查是否有人已经创建实例
          uniqueInstance = new Singleton();
        }
      }
    }
...
模板模式是一种行为设计模式，它在抽象类中写一个模板方法（规定了一系列流程），并将模板方法中用到的方法定义好（其中**钩子方法**可选，它可以改变模板方法中的步骤），然后由它的子类按需实现或重写，不管子类是怎么实现的，使用时都按照模板方法执行。
